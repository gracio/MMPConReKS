function plotMeanClustHeatmap(cv)
% plot heatmap of group var selection mean cluster size across cv runs. calibrate
...
for i=1:length(cv.groupMMPCoptions.method)
    limHigh = max(limHigh, max(max(cv.meanClustSize{i})));
end
...
for i=1:length(cv.groupMMPCoptions.method)
    figure('color',[1 1 1])
    imagesc(cv.meanClustSize{i},[limLow, limHigh])
    colormap('hot')
    axis square
    title(['simulated data: SVM CV mean cluster size, ' cv.groupMMPCoptions.method{i}],'fontsize',14)
    colorbar
    ...
    saveas(gcf,[cv.basicRunPath 'meanClustSizeHeatMap ' cv.groupMMPCoptions.method{i}],'fig')
end